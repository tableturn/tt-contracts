...
module.exports = async done => {
  try {
    ...
    await Promise.all([
      utils.promoteActor(people.pk2m, people.pk2m),
      utils.promoteIssuer(people.pk2m, people.issuer)
    ]);

    if (netId !== 18021982) {
      console.log('This is not production - making a few staging actors and governors...');
      await Promise.all([
        utils.promoteGovernor(people.pk2m, people.pierre),
        utils.promoteActor(people.pk2m, people.pierre),
        utils.promoteActor(people.pk2m, people.kevin)
      ]);
      ...
      if (balance.eq(new BN(0))) {
        ...
        await token.transfer(people.kevin, utils.convert('6050'), { from: people.pk2m });
        console.log(`Approving transfer...`);
        await transact.approve(people.pk2m, 0, { from: people.pierre });
      }
    }
