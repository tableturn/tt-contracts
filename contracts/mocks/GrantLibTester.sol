...
contract GrantLibTester {
  ...
  function make(bytes32 id, address owner, address recipient, uint256 maxAmount)
    external
    pure
    returns (GrantLib.Grant memory)
  {
    return GrantLib.make(id, owner, recipient, maxAmount);
  }
  ...
  function setSample1(
    ...
    sample1 = GrantLib.make(id, owner, recipient, maxAmount);
